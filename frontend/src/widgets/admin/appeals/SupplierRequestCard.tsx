import { SupplierRequestDto } from "@/domain";
...
interface Props {
    supplierRequest: SupplierRequestDto;
}
...
export const SupplierRequestCard = ({supplierRequest}: Props) => {
    const change = useUpdateAppealStatus(supplierRequest.id, repo);
    ...
    return (
        <Stack gap="sm" classNames={{root: styles.card}}>
            <Group justify="space-between">
                <Title order={3} classNames={{root: styles.subcardTitle}}>Компания: <strong>{supplierRequest.companyName}</strong></Title>
                <Select label="Статус" classNames={{
                    root: styles.field,
                    input: styles.fieldInput,
                    label: styles.fieldInputLabel,
                    error: styles.fieldInputError
                }} data={STATUS} value={supplierRequest.status} onChange={
                    (x) => onChange(x as AppealStatus)
                }></Select>
            </Group>
            <Divider size={2}></Divider>
            <Stack gap={5}>
                <Text classNames={{root: styles.entryTitle}}>Комментарий к заявке</Text>
                <Text classNames={{root: styles.question}}>{supplierRequest.content}</Text>
            </Stack>
            <Divider size={2}></Divider>
            <Stack gap={8}>
                ...
                <Group grow>
                    <Stack gap={5}>
                        ...
                        <Text classNames={{root: styles.entryValue}}>{supplierRequest.contactEmail}</Text>
                    </Stack>
                    <Stack gap={5}>
                        <Text classNames={{root: styles.entryTitle}}>Номер телефона</Text>
                        <Text classNames={{root: styles.entryValue}}>{supplierRequest.contactPhone}</Text>
                    </Stack>
                </Group>
                <Divider size={2}></Divider>
            </Stack>
