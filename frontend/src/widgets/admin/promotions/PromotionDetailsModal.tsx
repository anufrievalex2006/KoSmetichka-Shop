...
export const PromotionModal = ({id, opened, onClose}: Props) => {
    const {card, isLoading} = useContentCardDetails(id, repo);
    const formatDate = (date: string | null) => {
        return dayjs(date).add(7, "hours").format("DD.MM.YYYY в HH:mm");
    }
    return (
        <Modal title="Информация об акции" opened={opened} size={1000} onClose={onClose} centered>
            {isLoading ? (
                ...
                <Text c="red" fw={700} ta="center">Акция не найдена</Text>
            ) : (card.type !== "PROMO") ? (
                <Text c="red" fw={700} ta="center">Предоставленная карточка не является акцией</Text>
            ) : (
                <Group wrap="nowrap" gap="lg" align="flex-start">
                    <Image alt="Логотип акции информация" className={
                        styles.logo
                    } src={card.photoUrl || noImage}></Image>
                    <Stack flex={1} gap="lg">
                        <Title order={2} classNames={{root: styles.promoName}}>{card.title}</Title>
                        <Divider flex={1}></Divider>
                        <Text classNames={{root: styles.promoDescr}}>{card.description}</Text>
                        <Stack gap={8}>
